...
//lby_in.gat,63,93,4	script	宮廷魔術師の助手	893,{}
prontera.gat,150,120,4	script	宮廷魔術師の助手	893,{
		...
		//setarray '@mainF,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002;		//盾1種
	...
	case 1:	/* 超越更新をする */
		...
		switch(select(
			...
			setarray '@Lefthand,	1335,	1100,	0,	0,	0,	0,	0,	0,	0,	0,	0,	28784,	0,	0,	0;	//短剣片手剣片手斧
				/* 初期設定 */
				set '@refA,10;					//'@slotAの精錬値条件(10以上)
				set '@refB,99;					//'@slotBの精錬値条件(未使用)
			...
			setarray '@mainB,	19449,	19473,	19447,	5599,	5739,	5514,	5569,	19468,	19471,	5546,	15824,	19496,	15823,	400015;		//頭14種
				...
				/* 初期設定 */
				set '@refA,10;					//'@slotAの精錬値条件(10以上)初期条件
				set '@refB,99;					//'@slotBの精錬値条件(未使用)
				set '@refC,99;					//'@slotCの精錬値条件(未使用)
				set '@refD,99;					//'@slotDの精錬値条件(未使用)
			...
			case 7:	/*盾超越*/
			/* 盾は拡張性と記述統一でわかりやすくする */
			setarray '@mainF,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002,	460002;		//盾
			...
			switch(select(
				...
				""+getitemname('@mainF[8])+"",
				""+getitemname('@mainF[9])+"",
				""+getitemname('@mainF[10])+"",
				""+getitemname('@mainF[11])+"",
				""+getitemname('@mainF[12])+"",
				""+getitemname('@mainF[13])+""))	{
					case 1: mes "[" +strnpcinfo(1)+ "]"; mes "ご利用お待ちしています"; close;
					case 2: set '@EQ,0;	break;
					case 3: set '@EQ,1;	break;
					case 4: set '@EQ,2;	break;
	...
	switch(select(
		...
		switch(select(
			...
			""+getitemname('@mainA[14])+""))	{
				...
				/* 初期設定 */
				set '@refD,9;					//□□□■ '@slotD 第4スロット 精錬値条件(9以上)
				set '@refC,99;					//□□■□ '@slotC 第3スロット 精錬値条件(未使用)
				set '@refB,10;					//□■□□ '@slotB 第2スロット 精錬値条件(10以上)
				set '@refA,99;					//■□□□ '@slotA 第1スロット 精錬値条件(未使用)
				set '@pos,4;					//頭上1	頭中9	頭下10	鎧2	左盾3	右武4	肩5	靴6	左ア(1)7	右ア(2)8
